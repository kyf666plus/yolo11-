...
def split_safety_barrier_dataset(images_dir, labels_dir, output_dir, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1):
    """
    划分安全防护栏数据集
    """
    print("开始划分安全防护栏数据集...")
    ...
    print(f"找到 {len(valid_images)} 个有效的安全防护栏图片-标注对")
...
def create_safety_barrier_yaml(dataset_path, class_names, yaml_path):
    """
    创建安全防护栏数据集的YAML配置文件
    """
    config = {
    ...
    print(f"安全防护栏配置文件已创建: {yaml_path}")


def train_safety_barrier_model():
    """
    训练安全防护栏检测模型
    """
    print("\n开始训练安全防护栏检测模型")
    ...
    output_dir = 'datasets/safety_barrier_split'
    yaml_path = 'datasets/safety_barrier.yaml'
    ...
    if not classes:
        ...
        classes = ['safety_barrier']

    # 划分数据集
    splits = split_safety_barrier_dataset(images_dir, labels_dir, output_dir)
    if splits is None:
        return None

    # 创建配置文件
    create_safety_barrier_yaml(os.path.abspath(output_dir), classes, yaml_path)
    ...
    try:
        ...
        print("安全防护栏模型训练完成!")
        print(f"最佳权重保存在: runs/train/safety_barrier_detector/weights/best.pt")
...
def main():
    ...
    # 训练模型
    results = train_safety_barrier_model()

    if results:
        print("\n训练成功完成!")
        print("可以在以下位置找到结果:")
        print("- 训练日志: runs/train/safety_barrier_detector/")
        print("- 模型权重: runs/train/safety_barrier_detector/weights/")
        print("- 训练图表: runs/train/safety_barrier_detector/results.png")
    else:
        print("\n训练失败!")
