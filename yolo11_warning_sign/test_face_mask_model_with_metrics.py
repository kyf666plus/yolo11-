...
def generate_confusion_matrix(all_predictions, all_ground_truths, confidence_threshold, results_dir):
    """生成混淆矩阵（简化版：检测到 vs 未检测到）"""

    y_true = []  # 真实标签：0=无目标，1=有目标
    y_pred = []  # 预测标签：0=无目标，1=有目标

    for pred_boxes, gt_boxes in zip(all_predictions, all_ground_truths):
        # 真实标签
        has_gt = 1 if len(gt_boxes) > 0 else 0
        y_true.append(has_gt)

        # 预测标签（过滤置信度）
        filtered_preds = [box for box in pred_boxes if box['confidence'] >= confidence_threshold]
        has_pred = 1 if len(filtered_preds) > 0 else 0
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=['No Mask', 'Has Mask'],
                yticklabels=['No Mask', 'Has Mask'])
    plt.title(f'Confusion Matrix (Confidence Threshold: {confidence_threshold})')
    ...
    print(f"✅ 混淆矩阵保存: {results_dir / 'confusion_matrix.png'}")


def save_detailed_results(detailed_results, metrics_results, results_dir):
    """保存详细结果到JSON文件"""
    ...
    try:
        ...
        print(f"❌ 保存详细JSON失败: {e}")# 保存为简化版本
        try:
            summary_only = {
                'total_images': len(detailed_results),
                'best_metrics': metrics_results_clean[0] if metrics_results_clean else {},
                'all_thresholds': [m['confidence_threshold'] for m in metrics_results_clean],
                'all_f1_scores': [m['f1_score'] for m in metrics_results_clean]
            }
            with open(results_dir / 'summary_results.json', 'w', encoding='utf-8') as f:
                json.dump(summary_only, f, indent=2, ensure_ascii=False)
            print(f"✅ 简化结果保存: {results_dir / 'summary_results.json'}")
...
## 📈 混淆矩阵统计
- **真正例 (TP)**: {best_metrics['true_positives']} - 正确检测到的面罩
- **假正例 (FP)**: {best_metrics['false_positives']} - 误检测的面罩
- **假负例 (FN)**: {best_metrics['false_negatives']} - 漏检测的面罩

## 💡 性能解读
- **准确率 {best_metrics['precision'] * 100:.1f}%** 表示检测结果的可靠性
- **召回率 {best_metrics['recall'] * 100:.1f}%** 表示能检测到的面罩比例
...
def test_face_mask_model_with_metrics():
    """测试防护面罩检测模型并计算性能指标"""

    # 修正后的模型路径
    model_path = r"D:\集成\yolo11_warning_sign\runs\detect\runs\train\face_mask_detection\weights\best.pt"
    ...
    # 测试数据路径
    test_images_dir = Path(r"D:\数据集标注\防护面罩_yolo\images\test")
    test_labels_dir = Path(r"D:\数据集标注\防护面罩_yolo\labels\test")
    ...
    # 创建结果文件夹
    results_dir = Path("test_results/face_mask_metrics")
            ...
            if boxes is not None and len(boxes) > 0:
                for box in boxes:
                    ...
                    pred_boxes.append({
                        'class_id': class_id,
                        'confidence': conf,
                        'bbox': [float(x1), float(y1), float(x2), float(y2)]  # 确保是Python float
                    })
    ...
    if best_metrics:
        ...
        # 生成混淆矩阵
        try:
            generate_confusion_matrix(all_predictions, all_ground_truths, best_threshold, results_dir)
        except Exception as e:
            print(f"❌ 生成混淆矩阵失败: {e}")

        # 保存结果
        try:
            save_detailed_results(detailed_results, metrics_results, results_dir)
        except Exception as e:
            print(f"❌ 保存详细结果失败: {e}")
...
if __name__ == "__main__":
    test_face_mask_model_with_metrics()
